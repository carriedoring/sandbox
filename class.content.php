<?php
/**
 * Saves HTML produced by other classes which will be output to browser.
 * 
 * Other classes store HTML in this object in the properties $premain, $main and $postmain
...
class Content {
	...
	public function menu($title, $args = array()) {
		global $site;
		$site_menus = json_decode($site->site_menus, true);
	...
	private function create_menu_items($menu_item, $args) {
		...
		$site_url = defined('ASSETS_URL') ? ASSETS_URL : $site->site_url;
		
		// type cast array as object
		$menu = (object) $menu_item;
		// check if role has access
		if (in_array($user->role_id,explode('|',$menu->role_access))) {
			// check for full url
			$link = '<li';
			// check if current page
			if ($menu->url == $page->requested_url) {
			...
			if ($menu->url != "" && strpos($page->requested_url,$menu->url . '/') !== false) {
