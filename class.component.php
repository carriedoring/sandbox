...
class Component {
	 ...
	 * @return bool
	 */
	public function find_sub_components($requested_component, $page, $components, $sub_components) {
		return true;
	}
	 ...
	 * by sub_components, this would be where to add that sort of thing $page->something = "like this".
	 ...
	 * @return bool
	 */
	public function build_sub_components($each_component, $page) {
		return true;
	}
	 ...
	 * @return bool
	 */
	public function allow_sub_components($each_component, $page) {
		return true;
	}

	/**
	 * Generates links for this component.last component was the requested id, so generate links for this component
	...
	public function as_link($each_component, $page) {
		...
		$page->content->main .= '<div class="items-link"><a href="' . $page->site->site_url . '/' . $each_component->url . '">' . $title . '</a></div>'  . PHP_EOL;
	}
	 ...
	 */
	public function recipe_components($each_component, $page, $auto_create_reverse = null) {
		...
		if (isset($each_component->sub_recipe)) {
			...
			foreach ($each_component->sub_recipe as $key=>$each_recipe_component) {
				...
				if (class_exists($each_recipe_component['type']) === true) {
					...
					$component_type::recipe_manifestation((object) $each_recipe_component, $page);
				} else {
					// class has not been loaded, so do it now!
					// get list of components
					$activated_components = json_decode($page->site->activated_components, true);
				...
				if (isset($each_recipe_component['auto_create']) && $each_recipe_component['auto_create'] == "reverse") {
					...
					// clean up
					unset($auto_create[0]['components']);
					// recursive call back to this method with updated sub_components
					self::recipe_components($each_component, $page, $auto_create);
				...
				// set template to null if it hasn't been set already
				...
				if (isset($site->hooks['page_requested_components'])) {
					foreach($site->hooks['page_requested_components'] as $hook) {
						call_user_func($hook, $each_recipe_component);
					}
				}

				// check if user role can create this component
				if ($page->can_add((object) $each_recipe_component)) {
					...
					// check if component type has been activated
					if (isset($component_type)) {
						// call to add_component method
						$component_type::add_component((object) $each_recipe_component, $page);
					} else {
						// default to self
						self::add_component((object) $each_recipe_component, $page);
					}
				}
				...
				if (isset($auto_create_reverse)) {
					...
					if (!isset($page->recipe_manifestation_finish)) {
						$page->recipe_manifestation_finish = true;
						$auto_create[0]['type']::recipe_manifestation_finish((object) $each_recipe_component, $page);
					}
				} elseif (isset($recipe_manifestation) && $recipe_manifestation == $component_type) {
					$component_type::recipe_manifestation_finish((object) $each_recipe_component, $page);
				}

			}//end foreach
		}	//end checking if sub_recipe is set
	}
	 ...
	 * @param object $page
	 */
	public static function add_component($each_recipe_component, $page) {
	}

	/**
	 * Book end of recipe_components, which calls to add_component
	 * @param object $each_component
	 * @param object $page
	 */
	 ...
	 */
	public static function recipe_manifestation($each_recipe_component, $page) {
	}
	 ...
	 */
	public static function recipe_manifestation_finish($each_recipe_component, $page) {
	}
	 ...
	 * @param object $page
	 */
	public function revise_component($each_component, $page) {
		...
		self::edit_component($each_component, $page);
	}

	/**
	 * called from revised_comonent.
	 * @param object $each_component
	 * @param object $page
	 */
...
EOF;
	...
	protected function create_component($input) {
		...
		$auto_create_components = function($auto_create, $input, $direction) use (&$auto_create_components, $site, $create_component) {
			...
			}//end if (!empty($auto_create))

			return $input['parent_id'];

		};//end $auto_create_components()
	...
	protected function delete_component($input) {
		...
		// check that the user who created component is trying to edit it / or user role is admin
